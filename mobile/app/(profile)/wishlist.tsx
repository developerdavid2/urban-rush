      ...
      <View className="flex-1">
        ...
        <ScrollView
          ...
          contentContainerStyle={
            wishlist.length === 0 ? { flex: 1 } : { paddingBottom: 100 }
          }
          refreshControl={
            <RefreshControl
        ...
        {wishlist.length > 0 && (
          <View className="absolute bottom-0 left-0 right-0 bg-background px-6 py-4 border-t border-surface">
            <TouchableOpacity
              className="bg-emerald-500/20 rounded-2xl py-4 items-center flex-row justify-center"
              onPress={() => router.push(`/(tabs)`)}
              disabled={isRemovngWishlist}
            >
              <Text className="text-emerald-500 font-bold ml-3">
                Browse Products
              </Text>
            </TouchableOpacity>
          </View>
